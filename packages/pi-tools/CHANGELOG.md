...
- Added the `tgrep` tool: trigram-indexed exact content search through an external [tgrep](https://github.com/microsoft/tgrep) binary. Literal by default, `file:line:col:text` output, exit 1 reported as no-match. Registered at session start only when the binary resolves (`TGREP_BIN`, `tgrepBinPath`, or `PATH`), the workspace has a `.tgrep/` index, and `enableTgrep` is not `false`; mode-independent. Prompt guidelines steer on capability (exact/literal vs fuzzy/frecency), not repository size. Optional `tgrepTimeBudgetMs` / `TGREP_TIME_BUDGET_MS` (default 30s). Output truncation stays on a UTF-8 character boundary; mid-run abort reports `Operation aborted`.
