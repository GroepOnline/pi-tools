...
3. `Build & Publish` on a `v*` tag push publishes `@groeponline/pi-tools`, `@groeponline/fff-node`, `@groeponline/fff-bun`, plus GitHub Release assets. Manual `workflow_dispatch` republishes npm only.
